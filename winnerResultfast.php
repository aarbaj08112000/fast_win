...
$selectData=mysqli_query($con,"select * from `tbl_betting1` where `periodid`='".$periodid."' and `acceptrule`='on'");
$selectdataRow=mysqli_num_rows($selectData);
if($selectdataRow!='')
{
if($type1=='parity')
{
$chkparityQuery=mysqli_query($con,"select * from `tbl_betting1` where `periodid`='".$periodid."' and `tab`='parity'");
...
$sql= mysqli_query($con,"INSERT INTO `tbl_tempwinner1`(`periodid`,`number`,`color`,`total`, `type`) VALUES ('".$periodid."','".$x."','".$color."','".$total."','".$type1."')");	
}

//========final Result===================================================
if($switchstatus=='yes' && $switchtab=='parity')
{
$tempwinQuery=mysqli_query($con,"select `number`,`color`,`total` from `tbl_tempwinner1` where `periodid`='".$periodid."' and `number`='".$manualnumber."' and `color`='".$manualcolor."' and `type`='parity'");
	}

else{
$tempwinQuery=mysqli_query($con,"SELECT `number`,`color`,`total` from `tbl_tempwinner1` where `total` = (
select min(`total`)from `tbl_tempwinner1` where `total`>=0)and `periodid`='".$periodid."' and `type`='parity' limit 1");
}
...
if($type2=='sapre')
{
$chkparityQuery=mysqli_query($con,"select * from `tbl_betting1` where `periodid`='".$periodid."' and `tab`='sapre'");
$chkparityRow=mysqli_num_rows($chkparityQuery);
if($chkparityRow!=''){
//=========possibility for green type parity
for ($x = 0; $x <= 9; $x++) {
...
$sql= mysqli_query($con,"INSERT INTO `tbl_tempwinner1`(`periodid`,`number`,`color`,`total`, `type`) VALUES ('".$periodid."','".$x."','".$color."','".$total."','".$type2."')");	
}
//========final Result===================================================
if($switchstatus=='yes' && $switchtab=='sapre')
{
$tempwinQuery=mysqli_query($con,"select `number`,`color`,`total` from `tbl_tempwinner1` where `periodid`='".$periodid."' and `number`='".$manualnumber."' and `color`='".$manualcolor."' and `type`='sapre'");
	}
else{
$tempwinQuery=mysqli_query($con,"SELECT `number`,`color`,`total` from `tbl_tempwinner1` where `total` = (
select min(`total`)from `tbl_tempwinner1` where `total`>=0)and `periodid`='".$periodid."' and `type`='sapre' limit 1");
}
$tempwinResult=mysqli_fetch_array($tempwinQuery);
$tempNumber=$tempwinResult['number'];
$tempColor=$tempwinResult['color'];
...
$ResultQuery= mysqli_query($con,"INSERT INTO `tbl_result1`(`periodid`, `price`, `randomprice`, `result`, `randomresult`, `color`, `randomcolor`, `resulttype`, `tabtype`,`createdate`) VALUES ('".$periodid."','".$tempTotal."','".$randomPrice."','".$tempNumber."','".$randomNumber."','".$tempColor."','".$randomColor."','real','".$type2."','".$today."')");	
...
$ResultQuery= mysqli_query($con,"INSERT INTO `tbl_result1`(`periodid`, `price`, `randomprice`, `result`, `randomresult`, `color`, `randomcolor`, `resulttype`, `tabtype`,`createdate`) VALUES ('".$periodid."','0','".$randomPrice."','0','".$randomNumber."','','".$newcolor."','random','".$type2."','".$today."')");	
...
$sql= mysqli_query($con,"INSERT INTO `tbl_tempwinner1`(`periodid`,`number`,`color`,`total`, `type`) VALUES ('".$periodid."','".$x."','".$color."','".$total."','".$type3."')");	
}
//========final Result===================================================
if($switchstatus=='yes' && $switchtab=='bcone')
{
$tempwinQuery=mysqli_query($con,"select `number`,`color`,`total` from `tbl_tempwinner1` where `periodid`='".$periodid."' and `number`='".$manualnumber."' and `color`='".$manualcolor."' and `type`='bcone'");
	}
else{
$tempwinQuery=mysqli_query($con,"SELECT `number`,`color`,`total` from `tbl_tempwinner1` where `total` = (
select min(`total`)from `tbl_tempwinner1` where `total`>=0)and `periodid`='".$periodid."' and `type`='bcone' limit 1");
}
$tempwinResult=mysqli_fetch_array($tempwinQuery);
$tempNumber=$tempwinResult['number'];
$tempColor=$tempwinResult['color'];
...
$sql= mysqli_query($con,"INSERT INTO `tbl_tempwinner1`(`periodid`,`number`,`color`,`total`, `type`) VALUES ('".$periodid."','".$x."','".$color."','".$total."','".$type4."')");	
}
//========final Result===================================================
if($switchstatus=='yes' && $switchtab=='emerd')
{
$tempwinQuery=mysqli_query($con,"select `number`,`color`,`total` from `tbl_tempwinner1` where `periodid`='".$periodid."' and `number`='".$manualnumber."' and `color`='".$manualcolor."' and `type`='emerd'");
	}
else{
$tempwinQuery=mysqli_query($con,"SELECT `number`,`color`,`total` from `tbl_tempwinner1` where `total` = (
select min(`total`)from `tbl_tempwinner1` where `total`>=0)and `periodid`='".$periodid."' and `type`='emerd' limit 1");
}
$tempwinResult=mysqli_fetch_array($tempwinQuery);
$tempNumber=$tempwinResult['number'];
$tempColor=$tempwinResult['color'];
...
$truncateQuery=mysqli_query($con,"TRUNCATE TABLE `tbl_tempwinner1`");
